// Command burnin is the KubeMQ AMQP 1.0 burn-in soak harness (spec §9). It drives
// the embedded AMQP 1.0 connector through six workers (queues, events,
// events_store, commands, queries, credit_flow) over the native
// github.com/Azure/go-amqp client — NO kubemq-go, NO proto, NO gRPC. Phase 1
// runs queues + events; the other four are registered no-op stubs (Phase 2). It
// exposes a control API + Prometheus metrics on :8896 and runs a warmup →
// measure → drain → verdict lifecycle. Exit codes: 0 PASSED, 2
// PASSED_WITH_WARNINGS, 1 FAILED/config-error.
package main
...
import (
	...
	"github.com/kubemq-io/kubemq-amqp-1-0/burnin/api"
	"github.com/kubemq-io/kubemq-amqp-1-0/burnin/config"
	"github.com/kubemq-io/kubemq-amqp-1-0/burnin/engine"
	"github.com/kubemq-io/kubemq-amqp-1-0/burnin/metrics"
	"github.com/kubemq-io/kubemq-amqp-1-0/burnin/report"
	"github.com/kubemq-io/kubemq-amqp-1-0/burnin/server"
)
...
func (a *engineAdapter) RunReport() (*report.Summary, *report.Verdict) {
	...
	summary := &report.Summary{
		...
		SettleMode:    cfg.Amqp10.SettleMode,
	...
	for _, name := range config.AllWorkerNames {
		...
		ws := &report.WorkerStats{
			...
			FidelityFailures: snap.FidelityFailures,
	...
	// fidelity / throughput / latency P50-P999 / events-dropped that the report
	// verdict trims).
...
func printBanner(cfg *config.Config, logger *slog.Logger) {
	logger.Info("AMQP 1.0 burn-in starting",
		"mode", cfg.Mode,
		"broker", cfg.Broker.Address,
		"settle_mode", cfg.Amqp10.SettleMode,
