# Contributing to kubemq-amqp-1-0

Thanks for helping improve **kubemq-amqp-1-0** — the documentation, examples, and burn-in
harness for the KubeMQ **AMQP 1.0 (OASIS / ISO-IEC 19464)** connector. This repository teaches
developers to drive the connector from standard, off-the-shelf AMQP 1.0 (OASIS / ISO-IEC 19464)
client libraries. It ships **no installable package, no proto/gRPC bindings, and no published
client library** — so there is nothing to release or version-bump beyond the docs and examples
themselves.
...
- A running **KubeMQ server with the AMQP 1.0 (OASIS / ISO-IEC 19464) connector enabled** and
  reachable (see the [README](README.md) for the connection variable and default port).
- The toolchains for whichever example languages you touch (the examples span 6 (Go, Python,
  Java, C#, JS/TS, Rust; no Ruby)). Each language's exact version, install, and run steps live
  in [`examples/SHARED-CONVENTIONS.md`](examples/SHARED-CONVENTIONS.md).
...
| [`examples/`](examples/) | Runnable, per-pattern examples across 6 (Go, Python, Java, C#, JS/TS, Rust; no Ruby) — 13 variants × 6 languages = 78 example folders, using standard third-party AMQP 1.0 (OASIS / ISO-IEC 19464) libraries only. [`examples/SHARED-CONVENTIONS.md`](examples/SHARED-CONVENTIONS.md) is the single source of truth for the per-language loop. |
