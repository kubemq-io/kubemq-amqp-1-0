...
`github.com/Azure/go-amqp` client. The `x-opt-kubemq-group` receiver link
property places a subscriber in a named load-balancing group:
...
```bash
KUBEMQ_AMQP_URL=amqp://my-server:5672 go run ./events/consumer-group
```
...
1. **OPEN / BEGIN** — `amqp.Dial` (SASL ANONYMOUS by default). Each of the three
   subscribers runs on **its own `*Session` and goroutine** — go-amqp
   sessions/links are **not** concurrency-safe, so none is shared.
2. **ATTACH (3 receivers)** — each `NewReceiver("events/<ch>",
   {Credit:100, Properties:{"x-opt-kubemq-group": "<g>"}})` places the link in a
   load-balancing group. `g1a` and `g1b` use `"g1"`; `g2` uses `"g2"`.
...
6. **Assertions** — `g2` received all 30; `g1a + g1b` together received all 30
   with **no body delivered to both** (proves the group split, not duplicated,
   the stream) and both members got a non-empty share.
...
| receiver g1a / g1b (KubeMQ → client) | source `events/<ch>` | `first` (default) | client-granted standing `Credit:100` | deliveries pre-settled | **`x-opt-kubemq-group: "g1"`** | `Data` | round-robin within group g1 (no dup) |
| receiver g2 (KubeMQ → client) | source `events/<ch>` | `first` (default) | client-granted standing `Credit:100` | deliveries pre-settled | **`x-opt-kubemq-group: "g2"`** | `Data` | independent group ⇒ full stream |

## Gotchas

> **Group membership is a receiver link property, not an address.** All three
> receivers attach to the **same** `events/<ch>` source; the
> `x-opt-kubemq-group` property is what splits or duplicates the stream. Omit the
> property and a receiver is a standalone (lone) subscriber that gets the full
> stream — equivalent to its own one-member group.
...
- **One session/link per goroutine.** Each subscriber owns its own session — never
  share a `*Session`, `*Sender`, or `*Receiver` across goroutines.
...
- [queues/settlement-modes](../../queues/settlement-modes/) — at-most-once vs at-least-once
