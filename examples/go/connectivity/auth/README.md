...
**password is a KubeMQ JWT**, then run a `queues/<ch>` round-trip. Uses the
...
ANONYMOUS accepted): it reads the credentials from the environment and falls
back to ANONYMOUS when they are unset.
...
```bash
KUBEMQ_AMQP_URL=amqp://my-server:5672 go run ./connectivity/auth
```
...
1. **Select the mechanism** — the example reads `KUBEMQ_AMQP_USER` /
   `KUBEMQ_AMQP_JWT`. If a JWT is present it dials `SASLTypePlain(user, jwt)`;
   otherwise it dials `SASLTypeAnonymous()` (the dev-broker default) and prints a
   note.
2. **OPEN (SASL handshake)** — `amqp.Dial(ctx, url, &amqp.ConnOptions{SASLType:
   ...})`. The SASL exchange happens here. With auth **enabled**, the JWT in the
   PLAIN password must validate or `Dial` fails; with auth **disabled**, any
   credential is accepted.
3. **BEGIN** — `NewSession`.
4. **ATTACH (sender) + WRITE authz** — `NewSender("queues/<ch>")`. With
   authorization enabled, the connection's identity must hold a **WRITE** grant
   on this channel; otherwise the attach is refused with
   `amqp:unauthorized-access`.
5. **TRANSFER / DISPOSITION** — one unsettled `Send` blocks until the connector's
   `accepted` DISPOSITION.
6. **ATTACH (receiver) + READ authz** — `NewReceiver("queues/<ch>", {Credit:1})`.
   A denied identity's receiver attach is refused with
   `amqp:unauthorized-access` (the `TestAuthorizationReadDenied` contract).
...
|---|---|---|---|---|---|---|---|
| connection (SASL PLAIN) | n/a (`ConnOptions{SASLType: SASLTypePlain(user, jwt)}`) | n/a | n/a | bad/expired JWT ⇒ `Dial` fails (SASL `amqp:unauthorized-access`) | username audit-only; password = KubeMQ JWT | n/a | identity derived from the verified JWT when auth is enabled |
...
| receiver (KubeMQ → client) | source `queues/<ch>` | `first` (default) | client-granted `Credit:1` | `AcceptMessage` ⇒ AckRange (removed); denied attach ⇒ `amqp:unauthorized-access` | none | `Data` | READ authz checked at attach |

## Gotchas

> **Identity precedence depends on whether authentication is enabled.**
> - **Auth ENABLED:** the SASL PLAIN **password must be a valid KubeMQ JWT**.
>   The ClientID/identity is taken from the **verified token**; the SASL
>   **username is audit-only** (recorded with `auth.success` / `auth.failure`,
>   alongside mechanism + source IP). A bad/expired JWT fails the handshake
...
>
> Authorization (the WRITE/READ channel grants) is a **separate** Casbin layer:
> a denied attach is refused with `amqp:unauthorized-access`
...
- **The JWT is a secret.** Pass it via `KUBEMQ_AMQP_JWT` (env), never hard-code
  it or commit it.
